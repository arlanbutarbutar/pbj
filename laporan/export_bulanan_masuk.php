...
if (isset($_POST['month'])) {
    ...
    $tampil = mysqli_query($conn, "SELECT * FROM barang_masuk m, stock_barang s WHERE s.kode_stok = m.kode_stok AND MONTH(m.created_at) = $month AND YEAR(m.created_at) = $year");
}

$pdf->Cell(110, 30, 'Laporan Barang Masuk Bulan ' . $month_view . ' ' . $year, 0, 1, 'C');
...
$pdf->Cell(40, 10, 'Tahun Produksi', 1, 0, 'C');
...
while ($hasil = mysqli_fetch_array($tampil)) {
    ...
    $pdf->Cell(40, 10, $hasil['jumlah_barang'], 1, 0, 'C');
    $pdf->Cell(40, 10, $hasil['thn_produksi'], 1, 0, 'C');
    $pdf->Cell(40, 10, $hasil['tgl_masuk'], 1, 0, 'C');
