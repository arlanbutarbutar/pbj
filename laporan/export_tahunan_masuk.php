...
if (isset($_POST['year'])) {
    $year =$_POST['year'];
    $tampil = mysqli_query($conn, "SELECT * FROM barang_masuk m, stock_barang s WHERE s.kode_stok = m.kode_stok AND YEAR(m.created_at) = $year");
}

$pdf->Cell(110, 30, 'Laporan Barang Masuk Tahunan ' . $year, 0, 1, 'C');
